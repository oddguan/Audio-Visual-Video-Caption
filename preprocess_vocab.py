...
def main(params):
    ...
    for n, i in enumerate(videos):
        if 0 <= n < 8000:
            out['videos']['train'].append(int(i['id']))
            continue
        if 8000 <= n < 9000:
            out['videos']['val'].append(int(i['id']))
            continue
        if 9000 <= n < 10000:
            out['videos']['test'].append(int(i['id']))
            continue

    print('train videos amount: ', len(out['videos']['train']))
    print('validation videos amount: ', len(out['videos']['val']))
    print('test videos amount: ', len(out['videos']['test']))
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', type=str, default='data/videodatainfo_2017_ustc.json',
